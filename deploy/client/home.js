...
class Averages {
  ...
  render(courseObj) {
    ...
    ratingDataCol.innerHTML = rating === 0 ? "<b>N/A</b>" : "<b>" + rating + "</b>/5";
    ratingDataCol.classList.add('average');
    dataRow.appendChild(ratingDataCol);

    const diffDataCol = this.col.render();
    const difficulty = this.course.difficulty;
    diffDataCol.innerHTML = difficulty === 0 ? "<b>N/A</b>" : "<b>" + difficulty + "</b>/5";
    ...
    gradeDataCol.innerHTML = grade === 0 ? "<b>N/A</b>" : "<b>" + grade + "</b>";
    ...
    for (const review of courseReviews) {
      ...
      if (isUser) {
        ...
        delButton.addEventListener('click', async () => {
          const res = await crud.deleteReview(review.id);
          await startUserProfilePage();
        });
...
class LoginPage {
  ...
  render(isLogin) {
    ...
    submitInput.classList.add('review-button');
    ...
    const otherPageDiv = document.createElement('div');
    const otherPageInput = document.createElement('input');
    otherPageInput.type = 'button';
    otherPageInput.classList.add('review-button');
    otherPageInput.value = !isLogin ? 'Log In' : 'Register';
    otherPageInput.addEventListener('click', () => {
      isLogin? startRegisterPage() : startLoginPage();
    })
    otherPageDiv.appendChild(otherPageInput);

    form.appendChild(usernameDiv);
    form.appendChild(passwordDiv);
    form.appendChild(submitDiv);
    form.appendChild(otherPageDiv);
